...
namespace h2d
{
    ...
    namespace img
    {
        ...
        */
...
#ifdef HOMOG2D_USE_OPENCV
        template <>
        inline Image<cv::Mat>::Image(size_t width, size_t height)
        {
        ...
        template <>
        inline Image<SvgImage>::Image(size_t width, size_t height)
        {
    ...
    inline const char *getString(Dtype t)
    {
        const char *s = 0;
        switch (t)
        {
    ...
    namespace priv
    {
        ...
        template <typename T>
        inline std::pair<int, int> impl_dsize(const detail::DataFpType<T> &)
        {
        ...
        inline Dtype impl_dtype(const detail::DataFpType<float> &)
        {
            return Dtype::Float;
        }
        inline Dtype impl_dtype(const detail::DataFpType<double> &)
        {
            return Dtype::Double;
        }
        inline Dtype impl_dtype(const detail::DataFpType<long double> &)
        {
            return Dtype::LongDouble;
        }
        template <typename T>
        inline Dtype impl_dtype(const detail::DataFpType<T> &)
        {
            return Dtype::Other;
        }
#ifdef HOMOG2D_USE_TTMATH
        ...
        inline Dtype impl_dtype(const detail::DataFpType<ttmath::Big<M, E>> &)
        {
            return Dtype::Ttmath;
        }
        /// Implementation for ttmath types
        template <long unsigned int M, long unsigned int E>
        inline std::pair<int, int> impl_dsize(const detail::DataFpType<ttmath::Big<M, E>> &)
        {
    ...
    namespace thr
    {
        ...
        inline void printThresholds(std::ostream &f)
        {
    ...
    namespace detail
    {
        ...
        getCorrectPoints(const Point2d_<FPT> &p0, const Point2d_<FPT> &p1)
        {
            ...
            if (
                std::fabs(p0.getX() - p1.getX()) < thr::nullOrthogDistance() || std::fabs(p0.getY() - p1.getY()) < thr::nullOrthogDistance())
...
//------------------------------------------------------------------
    ...
    {
        template <typename>
        struct IsBigNumType : std::false_type
        {
        };
#ifdef HOMOG2D_USE_TTMATH
        template <int T1, int T2>
        struct IsBigNumType<ttmath::Big<T1, T2>> : std::true_type
        {
        };
#endif
    ...
    public:
        ...
        */
        ...
        Hmatrix_(const Hmatrix_<M, FPT> &other)
            : detail::Matrix_<FPT>(other), _hasChanged(true), _hmt(nullptr)
        {
...
/// Getter
        ...
        */
        bool operator==(const Hmatrix_ &h) const
        {
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    if (std::fabs(
                            static_cast<HOMOG2D_INUMTYPE>(data[i][j]) - h.value(i, j)) >= thr::nullDeter())
                        return false;
            return true;
        }
    ...
    namespace trait
    {
        ...
        template <typename T>
        struct IsDrawable : std::false_type
        {
        };
        template <typename T>
        struct IsDrawable<Circle_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsDrawable<FRect_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsDrawable<Segment_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsDrawable<Line2d_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsDrawable<Point2d_<T>> : std::true_type
        {
        };
        template <typename T1, typename T2>
        struct IsDrawable<base::PolylineBase<T1, T2>> : std::true_type
        {
        };
        ...
        template <typename T>
        struct IsShape<Circle_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsShape<FRect_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsShape<Segment_<T>> : std::true_type
        {
        };
        template <typename T>
        struct IsShape<Line2d_<T>> : std::true_type
        {
        };
        template <typename T1, typename T2>
        struct IsShape<base::PolylineBase<T1, T2>> : std::true_type
        {
        };
        ...
        template <typename T>
        struct HasArea<Circle_<T>> : std::true_type
        {
        };
        template <typename T>
        struct HasArea<FRect_<T>> : std::true_type
        {
        };
        template <typename T>
        struct HasArea<Ellipse_<T>> : std::true_type
        {
        };
        template <typename T>
        struct HasArea<base::PolylineBase<typename type::IsClosed, T>> : std::true_type
        {
        };

        /// This one is used in base;;PolylineBase::isInside()
        template <typename T>
        struct PolIsClosed : std::false_type
        {
        };
        template <typename T>
        struct PolIsClosed<base::PolylineBase<typename type::IsClosed, T>> : std::true_type
        {
        };
        ...
        struct IsContainer<std::vector<Ts...>> : std::true_type
        {
        };
        template <typename... Ts>
        struct IsContainer<std::list<Ts...>> : std::true_type
        {
        };
        ...
        template <typename T>
        struct IsArray<std::array<T, 3>> : std::true_type
        {
        };
        ...
        template <class T>
        struct IsSegment<Segment_<T>> : std::true_type
        {
        };
        template <class>
        ...
        template <class T>
        struct IsPoint<Point2d_<T>> : std::true_type
        {
        };
        ...
        */
        template <class>
        struct HasBB : std::false_type
        {
        };
        template <class T>
        struct HasBB<Ellipse_<T>> : std::true_type
        {
        };
        template <class T>
        struct HasBB<FRect_<T>> : std::true_type
        {
        };
        template <class T>
        struct HasBB<Circle_<T>> : std::true_type
        {
        };
        template <typename T1, typename T2>
        struct HasBB<base::PolylineBase<T1, T2>> : std::true_type
        {
        };
    ...
    public:
        ...
        template <
            ...
            typename std::enable_if<
                std::is_arithmetic<T>::value, T>::type * = nullptr>
        ...
        template <
            typename T,
            typename std::enable_if<
                trait::IsContainer<T>::value, T>::type * = nullptr>
        ...
        */
        template <
            ...
            typename T2,
            typename std::enable_if<
                (std::is_arithmetic<T1>::value && !std::is_same<T1, bool>::value), T1>::type * = nullptr>
        ...
        */
        template <
            typename PT,
            typename std::enable_if<
                !std::is_arithmetic<PT>::value, PT>::type * = nullptr>
        ...
        */
        template <
            typename T,
            typename std::enable_if<
                (std::is_arithmetic<T>::value && !std::is_same<T, bool>::value), T>::type * = nullptr>
        void set(T rad)
        {
            _radius = rad;
        }
        ...
        template <
            typename FPT2,
            typename std::enable_if<
                std::is_arithmetic<FPT2>::value, FPT2>::type * = nullptr>
            ...
            */
            ...
            template <
                ...
                typename std::enable_if<
                    (
                        !std::is_same<T, std::array<FPT, 3>>::value &&
                        (std::is_same<T, std::array<typename T::value_type, 3>>::value || std::is_same<T, std::vector<typename T::value_type>>::value)),
                    T>::type * = nullptr>
            LPBase(const T &arr)
            {
...
//------------------------------------------------------------------
    ...
    */
    ...
    template <
        typename T,
        typename std::enable_if<
            trait::IsContainer<T>::value, T>::type *>
    void
        ...
        public:
            ...
            */
            template <
                typename T,
                typename std::enable_if<
                    trait::PolIsClosed<T>::value, T>::type * = nullptr>
            bool
        ...
        */
        ...
        PolylineBase<PLT, FPT>::isConvex() const
        {
            ...
            for (size_t i = 0; i < vpts.size(); i++)
            {
                ...
                else if (sign != (crossproduct > 0 ? +1 : -1))
                    return false;
            }
            return true;
        }
    ...
    namespace detail
    {
        ...
        getPtLabel(const Point2d_<FPT> &pt, const Circle_<FPT2> &circle)
        {
            if (pt.isInside(circle))
                return PtTag::Inside;
            if (
                homog2d_abs(pt.distTo(circle.center()) - circle.radius()) < thr::nullDistance())
                return PtTag::OnEdge;
...
// not sure which one is the best...
    ...
    namespace base
    {
        ...
        */
        ...
        {
            return std::fabs(
                       static_cast<HOMOG2D_INUMTYPE>(_v[0]) * pt.getX() + static_cast<HOMOG2D_INUMTYPE>(_v[1]) * pt.getY() + static_cast<HOMOG2D_INUMTYPE>(_v[2]))
#ifndef HOMOG2D_USE_TTMATH
                   / std::hypot(_v[0], _v[1]);
#else
                ;
