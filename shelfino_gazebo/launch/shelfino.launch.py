...
def check_exists(context):
    ...
    if context.launch_configurations['use_rviz'] == "true" and \
        ...
        raise Exception("[{}] Rviz configuration `{}` does not exist".format(
            __file__, context.launch_configurations['use_rviz']))
...
def generate_launch_description():
    ...
    rviz_config_file = LaunchConfiguration('rviz_config_file',
                                           default=PythonExpression(["'", os.path.join(
                                               shelfino_desc_pkg, 'rviz', 'shelfino'), shelfino_id, ".rviz'"])
                                           )
    gazebo_world_file = LaunchConfiguration('gazebo_world_file',
                                            default=os.path.join(
                                                shelfino_gaze_pkg, 'worlds', 'hexagon.world')
                                            )

    shelfino_name = PythonExpression(["'", 'shelfino', shelfino_id, "'"])
    model_output = PythonExpression(["'", os.path.join(
        shelfino_desc_pkg, 'models', 'shelfino'), shelfino_id, ".sdf'"])
    ...
    def evaluate_rviz(context, *args, **kwargs):
        ...
        if context.launch_configurations['use_rviz'] == 'true':
            ...
            cr_path = os.path.join(shelfino_desc_pkg, 'rviz', 'shelfino') + \
                context.launch_configurations['shelfino_id'] + '.rviz'

            print("[{}] Replacing shelfinoX with {} from file {} to file {}".format(
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            arguments=[
                '-topic', PythonExpression(["'/", shelfino_name,
                                           "/robot_description", "'"]),
